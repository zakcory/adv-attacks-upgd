...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, n_batches, x_init=None, bs=250):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x.to(self.device) + self.eps * torch.ones_like(x,device=self.device).detach() * self.normalize(t)
        ...
        logits = torch.zeros(x.shape[0], 10).to(self.device)
        loss_indiv = torch.zeros(x.shape[0]).to(self.device)
        ...
        for i in tqdm(range(self.n_iter)):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(
                        x_adv + (x_adv_1 - x_adv) * a + grad2.mean(dim = 0) * (1 - a),
                        x - self.eps), x + self.eps), 0.0, 1.0)
                    
                    
                x_adv = x_adv_1 + 0.

            # calculate gradient w.r.t to pertubation
            x_adv.requires_grad_()
            grad = torch.zeros_like(x,device=self.device)
            logits = torch.zeros(x.shape[0], 10).to(self.device)
            loss_indiv = torch.zeros(x.shape[0]).to(self.device)
            ...
            if self.verbose:
                print('[m] iteration: {} - best loss: {:.6f} - robust accuracy: {:.2%}'.format(
                    i, loss_best.sum(), acc.float().mean()))
