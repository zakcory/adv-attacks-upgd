...
class AdvRunner:
    ...
    def run_clean_evaluation(self, x_orig, y_orig, n_examples, bs, n_batches, orig_device):
        ...
        for batch_idx in range(n_batches):
            ...
            x = x_orig[start_idx:end_idx, :].detach().clone().to(self.device)
            y = y_orig[start_idx:end_idx].detach().clone().to(self.device)
            output = self.model.forward(x)    # scores   
            correct_batch = y.eq(output.max(dim=1)[1]).detach().to(orig_device)   # output.max(dim=1) - [0]: max scores, [1]: max indices
